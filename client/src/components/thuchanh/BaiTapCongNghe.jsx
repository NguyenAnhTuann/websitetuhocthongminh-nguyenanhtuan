...
export default function BaiTapCongNghe() {
  // === DỮ LIỆU BÀI TẬP CÔNG NGHỆ ===
  const allExercises = [
    // --- BÀI TẬP CÔNG NGHỆ ---
    {
  ...
  // Màu chủ đạo mới cho Công Nghệ: Xám Tím Than (Slate)
  const primaryColor = "#475569"; // Màu slate-600
  const lightAccentColor = "bg-slate-200";
  const hoverColorClass = "hover:border-[#475569]/50";
  ...
  return (
    ...
    <section className="min-h-screen w-full bg-slate-50 relative overflow-hidden px-4 pt-32 pb-10">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
        ...
        {/* Màu xám xanh nhạt */}
        <div className="absolute top-[20%] -right-[10%] w-96 h-96 bg-gray-400 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-blob animation-delay-2000"></div>
      </div>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-4 mb-10">
          ...
          <div className="flex-1 text-center md:text-left">
            <h1 className={`text-3xl font-extrabold ${textPrimaryColorClass}`}>TƯƠNG TÁC CÔNG NGHỆ</h1>
            <p className="text-gray-500 text-sm mt-1">
              Tổng hợp tài liệu và bài tập về Kỹ năng số và Công nghệ thông tin
            </p>
          </div>
        </div>
              ...
              <button
                ...
                className={`w-10 h-10 rounded-lg font-semibold transition shadow-sm border ${
                  currentPage === page
                    ...
                    : `bg-white text-gray-700 border-gray-300 hover:bg-slate-100 hover:${textPrimaryColorClass}`
                }`}
              >
                {page}
              </button>
