...
import { LuAtom } from "react-icons/lu"; // Icon nguyên tử
    ...
    try {
      const res = await fetch(
        "https://websitetuhocthongminh-nguyenanhtuan.onrender.com/api/chat",
        {
          ...
          body: JSON.stringify({
            message: input,
            subject: "vatly", // Subject cho Vật Lý
          }),
        }
      );
    ...
    <section className="min-h-screen w-full flex flex-col items-center px-4 py-16 bg-white">
      ...
      >
        <div className="flex justify-center mb-4">
          <div className="w-20 h-20 bg-[#1c7c76] rounded-2xl flex items-center justify-center shadow-lg">
            <LuAtom className="text-white text-4xl" />
          </div>
        </div>
        ...
        <p className="text-gray-600 mt-3 max-w-2xl mx-auto text-base md:text-lg">
          Trợ lý AI môn Vật Lý – Giải đáp các hiện tượng tự nhiên, công thức Cơ học, 
          Điện từ học, Quang học, Nhiệt học và Vật lý nguyên tử.
        </p>
      </motion.div>
      ...
      <div className="w-full max-w-4xl mt-4">
        <div className="bg-white border border-gray-300 rounded-2xl px-4 py-3 shadow-sm flex items-center gap-3">
          <input
            value={input}
            placeholder="Nhập câu hỏi Vật Lý (VD: Phát biểu định luật II Newton...)"
        ...
        <p className="mt-2 text-xs text-gray-400 text-center">
          ChatBot chuyên môn Vật Lý – Powered by AI.
        </p>
      </div>
