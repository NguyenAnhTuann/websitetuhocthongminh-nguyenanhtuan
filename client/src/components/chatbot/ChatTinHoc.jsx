...
import { LuMonitor } from "react-icons/lu"; // Icon màn hình máy tính
    ...
    try {
      const res = await fetch(
        "https://websitetuhocthongminh-nguyenanhtuan.onrender.com/api/chat",
        {
          ...
          body: JSON.stringify({
            message: input,
            subject: "tinhoc", // Subject cho Tin Học
          }),
        }
      );
    ...
    <section className="min-h-screen w-full flex flex-col items-center px-4 py-16 bg-white">
      ...
      >
        ...
        <p className="text-gray-600 mt-3 max-w-2xl mx-auto text-base md:text-lg">
          Trợ lý AI môn Tin Học – Hỗ trợ lập trình (Pascal, C++, Python), giải thuật, 
          cấu trúc dữ liệu, cơ sở dữ liệu và Tin học văn phòng (Word, Excel, PowerPoint).
        </p>
      </motion.div>
      ...
      <div className="w-full max-w-4xl mt-4">
        <div className="bg-white border border-gray-300 rounded-2xl px-4 py-3 shadow-sm flex items-center gap-3">
          <input
            value={input}
            placeholder="Nhập câu hỏi (VD: Viết code Python tính giai thừa...)"
