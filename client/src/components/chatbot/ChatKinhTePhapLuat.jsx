...
import { LuScale } from "react-icons/lu"; // Icon cân công lý
    ...
    try {
      const res = await fetch(
        "https://websitetuhocthongminh-nguyenanhtuan.onrender.com/api/chat",
        {
          ...
          body: JSON.stringify({
            message: input,
            subject: "kinhtephapluat", // Nhớ khớp backend
          }),
        }
      );
    ...
    <section className="min-h-screen w-full flex flex-col items-center px-4 py-16 bg-white">
      ...
      >
        ...
        <h1 className="text-4xl md:text-5xl font-extrabold text-[#1c7c76]">
          ChatBot Môn Kinh Tế & Pháp Luật
        </h1>

        <p className="text-gray-600 mt-3 max-w-2xl mx-auto text-base md:text-lg">
          Trợ lý AI môn Kinh Tế & Pháp Luật – hỗ trợ khái niệm kinh tế, cung – cầu,
          thị trường, vai trò nhà nước, quyền – nghĩa vụ công dân, pháp luật Việt Nam
          và các bài tập tình huống thực tế.
        </p>
      </motion.div>
      ...
      <div className="w-full max-w-4xl mt-4">
        <div className="bg-white border border-gray-300 rounded-2xl px-4 py-3 shadow-sm flex items-center gap-3">
          <input
            value={input}
            placeholder="Nhập câu hỏi Kinh Tế hoặc Pháp Luật..."
