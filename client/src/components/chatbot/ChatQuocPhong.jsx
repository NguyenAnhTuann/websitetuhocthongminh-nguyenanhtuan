...
// Import icon môn Quốc Phòng từ react-icons/lu
import { LuShield } from "react-icons/lu";

export default function ChatQuocPhong() {
  ...
  useEffect(() => {
    setMessages([
      {
        sender: "bot",
        text: "Xin chào! 👋 Tôi là trợ lý AI chuyên về Giáo dục Quốc phòng. \n\nBạn cần tìm hiểu về đội ngũ hàng ngũ, kỹ thuật bắn súng, sơ cấp cứu hay kiến thức an ninh quốc gia không?",
      },
    ]);
  }, []);
      ...
      const res = await fetch(
        "https://websitetuhocthongminh-nguyenanhtuan.onrender.com/api/chat",
        {
          ...
          body: JSON.stringify({
            ...
            subject: "quocphong", // <--- Đổi thành môn Quốc Phòng
    ...
    <section className="min-h-screen w-full flex flex-col items-center px-4 py-16 bg-white">
      ...
      >
        <div className="flex justify-center mb-4">
          <div className="w-20 h-20 bg-[#1c7c76] rounded-2xl flex items-center justify-center shadow-lg">
            {/* Icon Quốc Phòng */}
            <LuShield className="text-white text-4xl" />
          </div>
        </div>
        ...
        <p className="text-gray-600 mt-3 max-w-2xl mx-auto text-base md:text-lg">
          Hỗ trợ kiến thức Giáo dục Quốc phòng, An ninh & Quân sự.
        </p>
      </motion.div>
      ...
      <div className="w-full max-w-4xl mt-4">
        ...
        <div className="bg-white border border-gray-300 rounded-2xl px-4 py-3 shadow-sm flex items-center gap-2">
          ...
          <input
            value={input}
            placeholder={isTyping ? "AI đang trả lời..." : "Hỏi về đội hình đội ngũ, tháo lắp súng..."}
        ...
        <p className="mt-2 text-xs text-gray-400 text-center">
          ChatBot chuyên môn Quốc Phòng – Powered by AI.
        </p>
      </div>
    </section>
  );
